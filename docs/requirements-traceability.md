...
- `npm run check`: TypeScript build passed; 488 tests passed, 0 failed after the connected St Albans workbench slice.
...
| S14 | Universal proposal→validate→apply immutable revision flow. | Journey/platform | Partial | Source ingestion and structured decisions now preview→validate→apply one versioned definition; journey/platform publication seal an exact validated immutable revision. Live repair remains the next seam. Stage 3. |
...
| G08 | Same Guard for creation/import/manual/AI/repair/every client. | Journey/Guard | Partial | Mac Describe/Quick, web journey, first JSON import and legacy platform/pilot publication run the server-owned Guard with AI absent; live repair and remaining import formats still need migration. Stage 3. |
