...
- `npm run check`: TypeScript build passed; 472 tests passed, 0 failed after the first connected slice.
...
- Git had no commits. The pre-implementation source was preserved without dependencies, generated output, build caches, local Xcode state, or secrets in baseline commit `38d92ec`.

Connected-slice evidence: `competition-journey.test.ts` creates a fresh draft, compiles 98 contests, receives a passing Guard report, binds an independent approval, and reopens the identical hash-bound revision through a fresh repository instance. `APIClientTests.swift` proves the Mac sends source facts, then only identity/version/acknowledgements across compile and approval; it never sends graph, schedule, simulation, or a Guard verdict. A live HTTP rehearsal produced approved revision `live-rehearsal-open.2ce6269b21`, Guard report `3acb7a4210388e467c6e2253be42935de3a851c362877f32ffbdef50cd07e782`, and 98 required/scheduled contests. Desktop and 390×844 browser inspections passed. The actual macOS Xcode target then built successfully and drove `Mac UI Rehearsal` through Review interpretation → assumptions/provenance → Compile & run Guard → explicit TSW104/TSW210 acknowledgement → Approve exact revision; the Mac read back 47 entrants/98 matches and opened `/competitions/mac-ui-rehearsal.ffd26fb5fb`, where Safari showed revision 1, `APPROVED`, Guard `PASSED`, 98/98, and bound approval.
...
| Mac creation | `apps/apple-client/.../{AppShell,FeatureViews}.swift` | Partial | The local compiler workspace now supports Describe/Quick, server interpretation review, editable required facts, visible assumptions, compile, Guard review, exact approval and web opening. Demo workspaces intentionally retain honest revision-zero local drafts. Import/duplicate/offline writes remain. |
...
| Organisation platform/event store | `packages/competition-engine/src/platform.ts`; `event-store.ts`; `postgres-event-store.ts` | Partial | Strong command/OCC/idempotency/publication primitives; compilation artefacts are not owned by a journey aggregate. |
...
| GUARD | `packages/competition-engine/src/{competition-guard,certification}.ts`; `competition-guard.test.ts`; publication in `platform.ts:903-927` |
...
| PLATFORM | `packages/competition-engine/src/{event-store,platform,platform-api,postgres-event-store}.ts`; platform/event/concurrency tests |
...
| M04 | A1 §4 | Immutable, versioned canonical IR with hashes and provenance. | J3/J6; schema; all | Implemented and connected | The journey persists a sealed proposal and immutable compiled revision with spec/graph/schedule/simulation/Guard/approval hashes; production storage adapter remains Stage 2. |
...
| M44 | A1 §44 | Command-oriented interpret→compile→validate→publish APIs. | J2–J6; journey/API | Partial | `/v1/competition-journey` now exposes strict create/revise/compile/approve/read commands and projects approved truth through `/v1/tournaments`; authenticated production composition is still required. Stage 2. |
...
| M50 | A1 §50 | Unit/integration/golden/property/regression/concurrency/replay/solver/compiler/mutation gates. | All; assurance | Partial | Broad 468-test suite; no mutation runner/config found. Permanent gate; add mutation evidence. |
...
| M60 | A1 §60 | An unseen format reaches approved live creation without reinterpretation or lost rules. | Product acceptance | Partial | Fresh P&K competition creation now completes create→compile→Guard→approval→shared retrieval. It is not an unseen format and does not yet enter live operations. Stage 2–3. |
...
| S14 | Universal proposal→validate→apply immutable revision flow. | Journey/platform | Partial | Revisions, publish and repair exist separately; unified seam missing. Stage 1–3. |
...
| G08 | Same Guard for creation/import/manual/AI/repair/every client. | Journey/Guard | Partial | Mac Describe/Quick and web journey creation now use the same Guard; imports, legacy platform publish and live repair still need the authoritative resolver. Stage 2–3. |
...
| G11 | Publication covers accounting/collisions/availability/dependencies/diff atomically. | Platform/Guard | Partial | Journey approval is atomic and server-built; the legacy platform publication command still accepts client artefacts and semantic diff is absent. Stage 2 critical migration. |
...
| G14 | Every generated blueprint must pass Guard before use/publication. | Journey/Guard | Partial | Connected journey blueprints cannot be approved unless server-run Guard passes; other legacy creation paths remain. Stage 2. |

### Critical Guard counterexample

The audit reproduced a coordinated omission against the baseline: removing one real P&K contest, its edges and schedule assignment, changing both graph counts to 97, clearing findings and omitting optional simulation previously returned `PASSED`. KCG003/KCG004 now independently rederive 98 required contests from the compiled spec, and the exact reproducer is a blocking regression test. The connected journey never accepts `guardInput`; Mac sends only source facts, then identity/version/acknowledgements. Residual risk remains in the legacy platform command (`platform.ts:243-248,903-917`), which still accepts client-supplied artefacts. Stage 2 must replace that boundary with a server-owned authoritative artefact resolver and add independent path/draw reconstruction.
...
| J06 | A4 Job 6 | Compare, prove, separately approve and atomically publish one exact revision. | Journey/platform; review | Partial | Demo/platform primitives; fresh path and authoritative artefact loading missing. Stage 1–2. |
...
| ISO09 | A5 isolation 9 | Public pages use minimal publication-derived projections, never organiser aggregate. | Public web | Missing | Current public attention is a separate demo singleton. Stage 2. |
...
| SURF01 | A8 | Install-free public web uses exact published minimal truth. | Public | Reference/demo only | Stage 2. |
| SURF02 | A8 | Mobile/Mac/web are job-specific clients of identical IDs/revisions/permissions/API/events. | Clients/journey | Partial | Connected Mac and web now share journey identity, revision and Guard hashes; demo/offline and public/live projections still diverge. Stage 2–3. |
...
4. Approval is actor-separated from the compiler component, hash-bound, durably written with optimistic version checks, and replayed identically by a fresh repository instance.
5. The Mac presents review, required questions, assumptions/provenance, compilation, Guard acknowledgement and approval as separate stages, then opens the exact same web identity.
6. The v1 portfolio/detail projection and responsive web page read the persisted journey revision; production v1 reads now fail closed without authorization.

YAML/XLSX/CSV/duplicate UI, generic format compilation, legacy platform publication migration, dynamic platform binding, public `/next`, live repair, durable offline transport, and production persistence/deployment remain explicitly traceable Stage 2–4 follow-ons; none is deleted or reclassified as complete. The next critical slice is replacing the legacy `PUBLISH_TOURNAMENT.guardInput` boundary with server-owned artefact loading before broadening formats.
