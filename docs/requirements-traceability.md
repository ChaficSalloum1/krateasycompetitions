...
- `npm run check`: TypeScript build passed; 493 tests passed, 0 failed after the connected live no-show slice.
...
- `xcodebuild -project apps/apple-client/TournamentOSClient.xcodeproj -scheme TournamentOSClient -configuration Debug -destination 'platform=macOS' -derivedDataPath /private/tmp/krateasy-live-no-show-mac-derived build`: `BUILD SUCCEEDED` after the unchanged sandbox-blocked command was rerun with Xcode cache access.
...
| Web creation proposal/workbench | `apps/compiler-web/src/{creation-proposal,competition-workbench,competition-journey}.ts` | Partial | Language/quick/JSON and multiple exact JSON sources persist under one versioned journey. The St Albans shape exposes facts, rules, provenance, assumptions, conflicts, missing decisions, unsupported semantics and registered edits. YAML/XLSX/CSV and general duplicate UI remain absent. |
...
| M42 | A1 §42 | Preserve Definition, Plan, Operational and Actual truth. | J8–J11; state | Partial | The connected no-show journey binds immutable definition/plan artefacts to versioned operational assignments and preserves completed/in-progress actual truth byte-for-byte; other platform live paths remain separate. Stage 3. |
...
| M50 | A1 §50 | Unit/integration/golden/property/regression/concurrency/replay/solver/compiler/mutation gates. | All; assurance | Broad 493-test TypeScript suite plus 23 Swift tests; no mutation runner/config found. Permanent gate; add mutation evidence. |
...
| ID | Requirement | Owner / surface | Status | Evidence, gap, phase |
|---|---|---|---|---|
| G01 | Independently validate roster/pools/counts/qualification/cups/byes/ties/normalisation/rematches. | Guard/core | Partial | St Albans ingestion independently closes pool/group/knockout counts and KCG003/KCG004 rederive total contest cardinality; full independent draw/path reconstruction remains. Stage 2. |
| G02 | Validate schedule collisions, calendars, duration, rest, window and quality. | Guard/schedule | Partial | St Albans validates 108 translated assignments against identity, collisions, participant paths, changing calendars, variable durations, hard stop and registered final resources; quality/resilience remains outside Guard. Stage 2. |
...
| G05 | Govern live-change authority, preservation, impact, notification and recovery. | Live Guard | Partial | The connected no-show path proves actual-truth preservation, affected scope, independent authority and exact targeted outbox intent; provider delivery and other incident policies remain. Stage 3. |
...
| G08 | Same Guard for creation/import/manual/AI/repair/every client. | Journey/Guard | Partial | Mac Describe/Quick, web journey, first JSON import, legacy publication and the connected no-show repair all rerun the server-owned Guard with AI absent; remaining import and incident formats still need migration. Stage 3. |
...
| G13 | Live Guard covers correction/delay/withdrawal/no-show/repair/notification/invalidation. | Live/platform | Partial | Withdrawal/no-show is connected through deterministic impact scope, walkover repair, replay/preservation checks, separate approval and targeted outbox publication; correction, delay and provider delivery remain. Stage 3. |
...
| J02 | A4 Job 2 | Describe/Quick/Import/Duplicate converge on one resumable draft and requirement ledger. | Journey; creation | Partial | Multiple exact JSON sources and structured edits now converge under one sealed, versioned journey; YAML/XLSX/CSV and duplicate UI remain. Stage 2. |
| J03 | A4 Job 3 | Explicit rules, assumptions, ambiguity, type checking and zero lost requirements. | Journey/Guard; review | Partial | The St Albans workbench exposes rules, provenance, untrusted claims, conflicts and eight missing critical decisions, then type-checks approved evidence. General authoring UI remains. Stage 2. |
| J04 | A4 Job 4 | Fair deterministic structure, unequal pools, qualification, draw and path proof. | Core; review | Implemented and connected | The St Albans fixture preserves 13 exact pools and explicitly approved percentage-normalised Konnect/Tower qualification through the existing graph/qualification/draw core. Broader review UX remains. |
...
| J08 | A4 Job 8 | Action-first control room with exact idempotent/offline/stale commands. | Operator/live | Partial | Connected routes accept strict server-attributed check-in/timing commands with optimistic concurrency and idempotent replay; control-room UI/offline transport remain. Stage 3. |
| J09 | A4 Job 9 | Typed disruption, scoped freeze, minimum-change repair, approval and targeted notify. | Live/repair | Implemented and connected | The St Albans no-show/withdrawal journey calculates affected scope, ranks two deterministic zero-move options, Guards each, explains consequences, requires independent approval and atomically targets affected consumers. |
...
| OPS04 | A7 §4 | Detect→freeze→rank choices→validate→approve→publish→communicate→learn. | Journey/live | Partial | The no-show path connects detect→non-mutating options→dual Guard→separate approval→operational publication→targeted outbox intents; provider delivery and learning remain. Stage 3. |
...
YAML/XLSX/CSV/duplicate UI, generic format compilation, full independent path/draw reconstruction, dynamic platform binding, public `/next`, court-outage/delay/correction repair, provider delivery, durable offline transport, and production persistence/deployment remain explicitly traceable Stage 2–4 follow-ons; none is deleted or reclassified as complete. The next critical slice is projecting published and live heads into personal/public truth with durable delivery and offline reconciliation before broadening formats.
