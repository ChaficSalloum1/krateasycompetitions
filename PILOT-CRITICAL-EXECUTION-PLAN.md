...
**Programme status:** Sprint 1 is complete. Sprint 2 now has one connected web/API/Mac workbench for Describe, Quick, exact JSON/YAML, CSV/XLSX entrant evidence and clean-edition duplication; raw sources, hashes, field provenance, quarantine, conflicts and reversible last-source removal survive restart. The Mac transmits exact source material and closure-bound duplication facts without interpreting competition truth locally; connected `/v1/tournaments` reads contain only authoritative journey records and the closed web view now creates a clean edition through the same exact closure-bound command. The connected organiser web page drives a source-ready named competition through server-owned CP-SAT compilation, independent Guard, exact acknowledgement, separate atomic publication and live activation; unavailable solver infrastructure fails before a candidate exists. The default web portfolio and Mac app now open only that connected authoritative workspace; reference UI, compiler examples and seeded profiles are confined to explicitly named non-production demo surfaces, and production fails closed when its real web projection is not configured. Guard 1.2 independently reconstructs the pilot's pool pair coverage, opening draw/byes, every round feeder and qualification dependency closure, then reconciles a bottom-up division/stage/pool/round contest-and-minute ledger with per-resource capacity. Its connected organiser pre-flight presents simple, detailed and technical views of the same report, stable correction guidance and an explicit six-level publication policy. The server-derived semantic and operational change set now survives compilation, approval, certificate issuance, atomic outbox publication, closure export and release assessment under one hash. Broader organisation deployment remains open. Sprint 3 has authoritative signed participant/public/organiser projections, durable targeted delivery, active-key rotation, participant revocation, a generic event QR and rate-limited privacy-safe recovery. An external provider/fallback and representative weak-network timing remain open. Sprint 4 has the connected command set, guarded no-show and court-outage repair, actual-result knockout progression and a server-derived delay/overrun revision. Sprint 5 has a server-signed, revision-scoped Mac offline event pack and participant lookup, a durable live/incident/stop command journal with explicit retry/conflict/reconciliation, a replayable Normal/Degraded/Paused/Stopped/Cancelled/Recovering authority state, typed incident log, command transfer and separately cleared restart, plus signed printable order-of-play, court, score, participant-QR and restoration materials. Sprint 6 has the connected authoritative close, replay, evidence export, isolated restore and clean-duplication journey over the completed St Albans state, executable markup/contrast/motion/forced-colour/print contracts and keyboard/200–400% reflow browser evidence across the live-information surfaces, plus one hash-bound release manifest that cannot confuse software evidence with external authority acceptance. The authorised emergency details, external delivery provider/fallback, full disconnect/manual restore rehearsal, named-assistive-technology/outdoor inspection, production restore, support assignment and staff dress rehearsal remain open and fail closed under their declared fallbacks.
...
**Current composition checkpoint (`14d5702`):** Organiser Studio now owns Competition Design, Run Assurance and Publish Review; Run Control owns action-first live commands and contextual guarded no-show Change Review; participant/public web and Close & Integrity Receipt remain the other two primary surfaces. The remaining deployment blockers are enumerated as owner/decision/evidence/fallback packages in [`docs/pilot-release-readiness.md`](./docs/pilot-release-readiness.md). This checkpoint does not mark the programme complete.

### Product boundary and pilot deployment lane

This document remains the version-controlled authority for the St Albans deployment gate; it is not the boundary of the Krateasy Competitions product. St Albans is the first permanent golden acceptance fixture and the first external-authority lane. Product engineering must also remove fixture-specific runtime assumptions and prove the same authoritative lifecycle with structurally different named competitions through the existing canonical model, compiler, independent Guard and revision chain. Venue facts, provider credentials, staff rehearsal and human approvals may block the St Albans event from proceeding, but they do not block safe product work that keeps those gates explicit and fail closed.
...
1. Unify Describe, Quick setup, JSON/YAML, CSV/XLSX and Duplicate behind one canonical creation command/model. **Connected for web/API/Mac:** the creator saves through `CompetitionJourney`; YAML normalises to the existing definition shape and CSV/XLSX reuse the existing entrant importer. The Mac sends exact supported files to those same boundaries and duplicates only from an authoritative closed head.
2. Add spreadsheet mapping, original-file preservation, quarantine, duplicate resolution, formula-injection protection and reversible commit. **Connected for the named product envelopes:** JSON, YAML, CSV and XLSX now enter through a staged Browse/drag-drop source intake with selected-file evidence and an explicit removal path; a primary source and up to seven corroborating files are previewed, compared and atomically saved as one versioned draft. Values-only `Entrants` sheets, exact CSV headers, byte/text hashes, active-content/formula/alias/duplicate/size/expansion quarantine, cross-source field conflicts and last-source removal are executable. Generic compilation independently reconstructs the accepted roster from preserved source bytes and rejects derived-workbench tampering. Mapping assistance remains open.
3. Persist requirement IDs, uncertainty classes, provenance and unresolved decisions. **Connected for St Albans and registered generic blueprints:** every St Albans entrant field retains per-source provenance and its eight critical decisions remain explicit; round-robin and single-elimination blueprints retain source-hash-bound facts and registered scoring, tiebreak, withdrawal, draw and timezone rules. Broader fresh-format uncertainty classes remain.
4. Add semantic diff for draft revisions and explicit preserved paths. **Connected for registered organiser decisions and source revisions:** the reviewed preview is retained through compilation and its server-derived semantic/operational change set is bound into approval, certificate, publication outbox, closure evidence and release assessment.
5. Connect participant unit and counts—individual, pair or team—without reference-event assumptions. **Connected for named pair entrants in the approved padel envelopes:** web/API/Mac sources preserve stable entrant and member identities, names, division and draw order; six-pair round robin and eight-pair single elimination compile and project those identities through the same journey without St Albans or 47-pair assumptions. Individual and team product journeys remain explicit follow-ons.
6. Connect organisation/workspace identity, roles and durable competition storage.
7. Retire or isolate overlapping guided `JsonValue`, legacy interpreter and local-only draft paths where migration evidence allows. **Connected for the pilot runtime:** the default web portfolio reads durable journey records only, the default Mac app exposes only its connected server session, and production refuses reference web/compiler/demo routes. Seeded profiles and local-only drafts remain preserved solely behind the explicit non-production `/demo` and test/reference clients.
8. Prove Mac and web reopen the same draft after a fresh process. **Connected for the pilot source envelope:** multi-source originals, hashes and provenance replay byte-identically after restart; the Mac initiates exact JSON/YAML/CSV/XLSX imports and subsequently reads the same authoritative journey DTO.
9. Drive the authoritative lifecycle from the connected organiser web page. **Connected for named pair envelopes:** the client submits only expected draft/revision identity and exact acknowledgements; the server compiles with the pinned CP-SAT solver, reruns Guard, publishes atomically and activates the live head. The eight-pair round-robin browser proof scheduled all 28 contests and opened exact revision-bound organiser/public projections with imported names; solver absence fails closed before approval.
...
| Breadth claims outrun connected evidence | Many partial claims, no reliable product | Prove the same lifecycle across named supported envelopes; keep every other format and sport explicit and fail closed |
...
- [x] One organisation, competition and operation identity persists across fresh processes.
...
- [x] Cross-organisation attacks fail closed.
...
The unchecked authority-owned gates are executable in `apps/compiler-web/src/pilot-release-readiness.ts` and documented in `docs/pilot-release-readiness.md`. The release scope is bound to the verified server-owned closure bundle; every missing or invalid acceptance reports its required owner and immutable safe fallback. The current truthful release result is `BLOCKED_EXTERNAL_AUTHORITY`, not pilot-ready.
